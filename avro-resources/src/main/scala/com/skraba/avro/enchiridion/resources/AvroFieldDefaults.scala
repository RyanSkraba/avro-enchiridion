...
object AvroFieldDefaults {
   ...
   */
  case class FieldDefaultCfg(tag: String,
                             fieldType: JsValue,
                             fieldDefault: JsValue) {

    def unionNullFirst(tagSuffix: String = "UnionNullFirst"): FieldDefaultCfg = FieldDefaultCfg(
      tag + tagSuffix, Json.arr("null", fieldType), fieldDefault)

    def unionNullLast(tagSuffix: String = "UnionNullLast"): FieldDefaultCfg = FieldDefaultCfg(
      tag + tagSuffix, Json.arr(fieldType, "null"), fieldDefault)
  ...
  object FieldDefaultCfg {
    def simpleType(fieldType: String)(tag: String, fieldDefault: JsValue): FieldDefaultCfg =
      FieldDefaultCfg(tag, JsString(fieldType), fieldDefault)
  }

  val AllNumericDefaults: Iterable[FieldDefaultCfg] = NumericValues.AllJson.flatMap {
    case (tag, v: JsValue) => List(
  ...
  def main(args: Array[String]) {
    ...
    dst.resolve("field-defaults-good.txt").toFile.writeAll(
      Valid.map { case (tag, json) => tag + ":" + Json.stringify(json) + "\n" }.toSeq: _*)
    dst.resolve("field-defaults-bad.txt").toFile.writeAll(
